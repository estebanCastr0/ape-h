...
export class PromedioComponent {
  ...
  calculateAverage() {
    ...
    console.log(this.cortes.length)
  ...
  addCorteField() {
    if(this.cortes.length >3){
      console.log("Porque agregar más")
    }
    else{
      this.cortes.push(0);
      this.values.push(0);
  ...
  calcularPredictivo() {
    ...
    if(this.cortes[0] > this.cortes.length) {
      this.weightedAverage = null;
      this.errorMessage = "La cantidad de cortes no es el indicado.";
    } 
    else if(verif>=1){
    var final = 0;
